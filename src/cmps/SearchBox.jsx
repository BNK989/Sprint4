...
export function SearchBox({neverHide = false, inHeader = true}) {
    ...
    return (
        <form className='search-form flex' hidden={isHidden} onSubmit={handleSubmit}>
            <input onChange={handleChange} type='search' className={`search-input ${inHeader ? 'w-[88.8%]' : 'w-96'} border rounded-tl-sm rounded-bl-sm p-1 font-thin`} autoComplete='on' placeholder={ !inHeader ? 'Search for any service...' : 'What service are you looking for today?' } />
