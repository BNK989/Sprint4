...
export function SearchBox({neverHide = false, inHeader = true}) {
    ...
    return (
        <form className='search-form flex' hidden={isHidden} onSubmit={handleSubmit}>
            <input onChange={handleChange} type='search' className={`${inHeader ? 'w-[88.8%]' : 'w-96'} border  h-10 rounded-tl-sm rounded-bl-md p-1`} autoComplete='off' placeholder='Search for any service...' />
